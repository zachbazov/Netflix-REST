...
const protect = catchAsync(async (req, res, next) => {
    ...
    if (!decodedUser) {
        const message =
            'No match for a user matching this token.';
        const appError = new AppError(message, 401);
...
const isSignedIn = async (req, res, next) => {
    if (req.cookies.jwt) {
        try {
            ...
            if (
                decodedUser.changedPasswordAfter(
                    decoded.iat
                )
            ) {
                return next();
...
const dispatchSignToken = async (
    ...
    res.cookie('jwt', token, {
        expires: new Date(
            Date.now() +
                process.env.JWT_COOKIE_EXPIRATION_PERIOD *
                    24 *
                    60 *
                    60 *
                    1000
    ...
    async (req, res, next) => {
        ...
        try {
            await dispatch({
                email: user.email,
                subject:
                    'Your password reset token. will be valid for 10min.',
                message
            });
            ...
            res.status(200).json({
                status: 'success',
                message:
                    'Password reset token has been to the provided email.'
            });
        } catch (err) {
